                    ...
                    for i in range(len(FTLAvailable)):
                        for j in range(3):
                            if Logistics[LNum][Orders[index][2]][8 + FTLAvailable[i] * 3 + j] != '' and \
                                    Logistics[LNum][Orders[index][2]][
                                        8 + FTLAvailable[i] * 3 + j] < mincost:  # 报价不为空且低于目前最小报价
                                LogisticSelect = LNum
...
def MultiOrders(Orders, index, Logistics, LTL, FTL, FTLRatio):
    ...
    if modelindex == 1:
        ...
        m_list = list(m_list)
        # 初始化求解器
        model = pywraplp.Solver('SkuShelf', pywraplp.Solver.SCIP_MIXED_INTEGER_PROGRAMMING)
        x = dict()
        y = dict()
        for key, value in XINDEX.items():
            x[key] = model.IntVar(lb=0, ub=1, name=str(key) + '_x')
        for key, value in PVM.items():
            y[key] = model.IntVar(lb=0, ub=1, name=str(key) + '_y')
        ...
        for i in OrdersOpt.keys():
            if OrdersOpt[i] == 1:
                ...
                for key in x.keys():
                    if key[0] == i:
                        tmp_list.append(x[key])
                model.Add(model.Sum(tmp_list) == 1)
        ...
        for p in p_list:
            for v in v_list:
                for m in m_list:
                    tmp_list = []
                    for i in OrdersOpt.keys():
                        if (i, p, v, m) in x.keys():
                            tmp_list.append(W[(i, p, v, m)] * x[(i, p, v, m)])
                    if v < 8:
                        if (p, v, m) in PVM.keys():
                            model.Add(model.Sum(tmp_list) <= PVM[(p, v, m)] * y[(p, v, m)])
                    else:
                        if (p, v, m) in PVM.keys():
                            model.Add(model.Sum(tmp_list) <= PVM[(p, v, m)] * y[(p, v, m)])
                            model.Add(model.Sum(tmp_list) >= FTLRatio * PVM[(p, v, m)] * y[(p, v, m)])
                ...
                for m in m_list:
                    ...
                    for i in OrdersOpt.keys():
                        if (i, p, v, m) in x.keys():
                            tmp_list.append(W[(i, p, v, m)] * x[(i, p, v, m)])
                    if (p, v, m) in PVM.keys():
                        model.Add(model.Sum(tmp_list) <= len(Orders[index][7]) * y[(p, v, m)])
        ...
        for i in XINDEX.keys():
            ...
            if v > 0.9:
                ...
                for j in XINDEX.keys():
                    if v > 0.9 and i[0] != j[0] and i[1] == j[1] and i[2] == j[2] and i[3] == j[3]:
